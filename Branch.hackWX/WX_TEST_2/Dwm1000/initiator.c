...
u8 Brd_Msg(u8 *data, u8 length, u8 TargetID)
{
	u8 i, IF_FAIL_SENDING = 0xFF;
	static u8 brd_msg[] = {0x7A, MyID, 0, 0x00, 0, 0x00, 0, 0x00, 0, 0x0D, 0x0A};
	brd_msg[2] = TargetID;
  ...
  dwt_setrxtimeout(7000);
	...
	dwt_writetxdata(length + 5, brd_msg, 0);
	dwt_writetxfctrl(length + 5, 0);
...
//u8 Brd_Msg(u8 *data, u8 length)
//{
//	u8 i, IF_FAIL_SENDING = 0xFF;
//	static u8 brd_msg[] = {0x7A, MyID, 0x00, 0, 0x00, 0, 0x00, 0, 0x0D, 0x0A};
//	
//	if(length > 6)
//		return 0xFA;
//	
...
//  dwt_setrxtimeout(RESP_RX_TIMEOUT_UUS);
//	
//	dwt_writetxdata(sizeof(length + 4), brd_msg, 0);
//	dwt_writetxfctrl(sizeof(length + 4), 0);
//    
