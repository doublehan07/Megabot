...
static u8 rpy_msg[] = {0x7B, MyID, 0, 0, 0};
...
u8 Receptor_Ranging(u16 *upperDist)
{
...
u8 Receive_BrdMsg(u16 *d01, u16 *d02, u16 *d03, u16 *d12, u16 *d13, u16 *d23)
{
	u8 IF_FAIL_RECEIVING = 0xFF;
	...
	if (status_reg & SYS_STATUS_RXFCG)
	{
		...
		if (rx_buffer[0] == 0x7A && rx_buffer[2] == MyID)
		{
			rpy_msg[2] = rx_buffer[1];
			dwt_writetxdata(sizeof(rpy_msg), rpy_msg, 0);
			dwt_writetxfctrl(sizeof(rpy_msg), 0);		
			dwt_starttx(DWT_START_TX_IMMEDIATE);
			
			if(rx_buffer[1] == 0x03)
			{
				*d03 = rx_buffer[3] << 8 | rx_buffer[4];
				*d13 = rx_buffer[5] << 8 | rx_buffer[6];
				*d23 = rx_buffer[7] << 8 | rx_buffer[8];
			}
			else if(rx_buffer[1] == 0x02)
			{
				*d02 = rx_buffer[3] << 8 | rx_buffer[4];
				*d12 = rx_buffer[5] << 8 | rx_buffer[6];
			}
			else if(rx_buffer[1] == 0x01)
			{
				*d01 = rx_buffer[3] << 8 | rx_buffer[4];
			}
...
//			if(rx_buffer[1] == 0x03)
//			{
//				*d03 = rx_buffer[2] << 8 | rx_buffer[3];
//				*d13 = rx_buffer[4] << 8 | rx_buffer[5];
//				*d23 = rx_buffer[6] << 8 | rx_buffer[7];
//			}
//			else if(rx_buffer[1] == 0x02)
//			{
//				*d02 = rx_buffer[2] << 8 | rx_buffer[3];
//				*d12 = rx_buffer[4] << 8 | rx_buffer[5];
//			}
//			else if(rx_buffer[1] == 0x01)
//			{
//				*d01 = rx_buffer[2] << 8 | rx_buffer[3];
//			}
//			IF_FAIL_RECEIVING = 0;
//		}
//	}
