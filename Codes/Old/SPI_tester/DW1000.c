...
// Interrupt
idata void (*callbackRX)(void), (*callbackTX)(void);                                          // function pointer to callback which is called when successfull RX took place
...
void ISR() {
    uint64_t status;
		status = getStatus();
    if (status.low32 & 0x4000) {                                          // a frame was received
        callbackRX();
        writeRegister16(DW1000_SYS_STATUS, 0, 0x6F00);              // clearing of receiving status bits
    }
    if (status.low32 & 0x80) {                                            // sending complete
        callbackTX();
