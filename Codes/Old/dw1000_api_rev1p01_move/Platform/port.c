...
int RCC_Configuration(void)
{
	...
	if(HSEStartUpStatus != ERROR)
	{
		...
		/* Configure PLLs *********************************************************/
		/* PLLCLK = HSE(8M)/8*336/4 = 84 MHz */
		RCC_PLLConfig(RCC_PLLSource_HSE, 8, 336, 4, 6);
		/* Enable PLL */ 
		RCC_PLLCmd(ENABLE);
...
int SPI_Configuration(void)
{
  ...
  GPIO_Init(SPIx_GPIO, &GPIO_InitStructure);
	GPIO_PinAFConfig(SPIx_GPIO,SPIx_SCK_Source,SPIx_AF);
	GPIO_PinAFConfig(SPIx_GPIO,SPIx_MISO_Source,SPIx_AF);
	GPIO_PinAFConfig(SPIx_GPIO,SPIx_MOSI_Source,SPIx_AF);
