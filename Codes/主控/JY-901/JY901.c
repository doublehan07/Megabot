...
unsigned char DMA_JY_Buffer[11];
...
{
	if (DMA_JY_Buffer[0] != 0x55)  //数据头不对，则重新开始寻找0x55数据头
	{
		DMA_JY901_Find_First_Data();
	}
	else
	{
		switch(DMA_JY_Buffer[1])
		{
			//memcpy为编译器自带的内存拷贝函数，需引用"string.h"，将接收缓冲区的字符拷贝到数据共同体里面，从而实现数据的解析。
			case 0x50:	memcpy(&stcTime, &DMA_JY_Buffer[2], 8); break;			//时间
			case 0x51:	memcpy(&stcAcc, &DMA_JY_Buffer[2], 8); break;			//加速度
			case 0x52:	memcpy(&stcGyro, &DMA_JY_Buffer[2], 8);						//角速度
									sAngle += stcGyro.w[2] / 163.84;
									break;
			case 0x53:	memcpy(&stcAngle, &DMA_JY_Buffer[2], 8); break;		//角度
			case 0x54:	memcpy(&stcMag, &DMA_JY_Buffer[2], 8); break;			//磁场
			case 0x55:	memcpy(&stcDStatus, &DMA_JY_Buffer[2], 8); break;	//端口状态数据
			case 0x56:	memcpy(&stcPress, &DMA_JY_Buffer[2], 8); break;		//气压
			case 0x57:	memcpy(&stcLonLat, &DMA_JY_Buffer[2], 8); break;		//经纬度
			case 0x58:	memcpy(&stcGPSV, &DMA_JY_Buffer[2], 8); break;			//地速
			case 0x59:	memcpy(&stcQuat, &DMA_JY_Buffer[2], 8); break;			//四元数
			default:	break;
		}
	}
}
