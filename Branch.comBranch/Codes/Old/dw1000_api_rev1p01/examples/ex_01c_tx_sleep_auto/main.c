...
#define APP_NAME "TX AUTO SLP v1.1"
...
int main(void)
{
    ...
    /* Wake DW1000 up in case it is asleep.
     * As DW1000 is not waken up by any reset, we can get at this point with it being asleep (e.g. after a physical reset of the board). Note that
     * this may be true for any other application but we pay special attention here as this example is precisely about sleeping. */
    ...
    while (1)
    {
        ...
        /* In this example, there is no point in waiting for TX frame sent event as it will not be possible to access any DW1000 registers once the
         * frame is sent and the DW1000 automatically put to sleep at the same time. */
